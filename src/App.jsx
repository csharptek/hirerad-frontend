...
<<<<<<< HEAD
  ...
  const [mode, setMode] = useState("company"); // "company" | "person"

  // ── Shared enriching state ────────────────────────────────────
  const [enrichingIds, setEnrichingIds] = useState({}); // { apollo_id: true }
  const [bulkEnriching, setBulkEnriching] = useState(false);
  const [copied, setCopied] = useState(null);
  const copyText = (text, key) => { navigator.clipboard?.writeText(text); setCopied(key); setTimeout(()=>setCopied(null),1500); };

  // ── Person Lookup state ───────────────────────────────────────
  const [pq, setPq]               = useState("");  // free text query
  ...
  const searchPeople = async (page = 1) => {
    if (!apolloKey) { setPError("Add Apollo.io API key in Settings"); return; }
    if (!pq.trim()) { setPError("Enter a search query"); return; }
    setPSearching(true); setPError(""); if (page===1) setPResults([]); setPSelected({});
    try {
      const res  = await fetch(`${API_BASE}/apollo/person-search`, {
        method:"POST", headers: proxyHeaders,
        body: JSON.stringify({ q: pq.trim(), page, per_page: PER_PAGE }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Search failed");
      setPResults(data.people || []);
  ...
  // ── Company People state ──────────────────────────────────────
  const [cq, setCq]               = useState("");
  const [cSeniorities, setCseniorities] = useState([]); // selected filter groups
  ...
  const SENIORITY_GROUPS = [
    { label:"👑 C-Suite",   values:["c_suite"],                       color:"#7c3aed" },
    { label:"🚀 Founders",  values:["owner","founder"],               color:"#059669" },
    { label:"📊 Directors", values:["director"],                      color:"#2563eb" },
    { label:"📈 VPs",       values:["vp"],                            color:"#d97706" },
    { label:"🛠 Managers",  values:["manager","senior_manager"],      color:"#0891b2" },
    { label:"👥 All",       values:[],                                color:"#64748b" },
  ];

  const toggleSeniority = (group) => {
    if (group.values.length === 0) { setCseniorities([]); return; } // "All" clears
    setCseniorities(prev => {
      const already = group.values.every(v => prev.includes(v));
      if (already) return prev.filter(v => !group.values.includes(v));
      return [...new Set([...prev, ...group.values])];
    });
  };
  ...
  // ── Bulk Enrich (shared for both modes) ──────────────────────
  // Takes array of person objects, calls bulk-enrich, updates results in place
  const bulkEnrich = async (people, source, onUpdate) => {
    if (!people.length) return;
    setBulkEnriching(true);
    // Mark all as enriching
    const ids = {};
    people.forEach(p => { ids[p.apollo_id] = true; });
    setEnrichingIds(ids);
    ...
    try {
      ...
      for (let i = 0; i < people.length; i += 10) {
        ...
        const res   = await fetch(`${API_BASE}/apollo/bulk-enrich`, {
          method: "POST", headers: proxyHeaders,
          body: JSON.stringify({ people: batch, source }),
        });
        const data  = await res.json();
        if (data.results) onUpdate(data.results);
      }
    } catch(err) { console.error("Bulk enrich error:", err.message); }

    setEnrichingIds({});
    setBulkEnriching(false);
  };

  // Enrich selected people in person lookup
  const enrichPersonLookup = () => {
    const selected = pResults.filter((_,i) => pSelected[i]);
    const targets  = selected.length ? selected : pResults;
    bulkEnrich(targets, "person_lookup", (results) => {
      setPResults(prev => {
        const updated = [...prev];
        results.forEach(r => {
          const idx = updated.findIndex(p => p.apollo_id === r.apollo_id);
  ...
  // Enrich selected people in company lookup
  const enrichCompany = () => {
    const selected = cResults.filter((_,i) => cSelected[i]);
    const targets  = selected.length ? selected : cResults;
    bulkEnrich(targets, "company_lookup", (results) => {
      setCResults(prev => {
        const updated = [...prev];
        results.forEach(r => {
          const idx = updated.findIndex(p => p.apollo_id === r.apollo_id);
  ...
  // ── Shared person row renderer ───────────────────────────────
  const PersonRow = ({ p, i, selected, onToggle, showEnrich, onEnrichSingle }) => {
    const isEnriching = enrichingIds[p.apollo_id];
    return (
      <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
        ...
        <div style={{ flex:1, background:"var(--surface2)", border:`1px solid ${p.enriched?"#86efac":"var(--border)"}`, borderRadius:10, padding:"12px 14px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:6 }}>
            ...
            <div style={{ display:"flex", gap:4, alignItems:"center", flexWrap:"wrap" }}>
              {p.enriched
                ...
                : isEnriching
                  ? <Badge variant="warning"><Spinner size={10}/> Enriching...</Badge>
                  : <Badge variant="warning">Not enriched</Badge>
              }
            </div>
          </div>
          ...
          {/* Email */}
          {p.email && (
            <div style={{ marginTop:8, display:"flex", alignItems:"center", gap:8, background:"#dbeafe", border:"1px solid #93c5fd", borderRadius:6, padding:"6px 10px" }}>
              <span style={{ fontFamily:"var(--mono)", fontSize:11, color:"var(--accent)", flex:1 }}>✉ {p.email}</span>
              <button onClick={()=>copyText(p.email,`e-${p.apollo_id}`)} style={{ background:"transparent", border:"none", cursor:"pointer", color:copied===`e-${p.apollo_id}`?"var(--accent3)":"var(--muted)", fontSize:11, fontFamily:"var(--font)", fontWeight:700 }}>
                {copied===`e-${p.apollo_id}` ? "✓" : "📋"}
              </button>
            </div>
          )}
          ...
          {/* Phone */}
          {p.phone && (
            <div style={{ marginTop:4, display:"flex", alignItems:"center", gap:8, background:"#f0fdf4", border:"1px solid #86efac", borderRadius:6, padding:"6px 10px" }}>
              <span style={{ fontFamily:"var(--mono)", fontSize:11, color:"#166534", flex:1 }}>📞 {p.phone}</span>
              <button onClick={()=>copyText(p.phone,`ph-${p.apollo_id}`)} style={{ background:"transparent", border:"none", cursor:"pointer", color:copied===`ph-${p.apollo_id}`?"var(--accent3)":"var(--muted)", fontSize:11, fontFamily:"var(--font)", fontWeight:700 }}>
                {copied===`ph-${p.apollo_id}` ? "✓" : "📋"}
              </button>
            </div>
          )}
          ...
          {/* Auto-saved notice */}
          {p.enriched && (
            <div style={{ marginTop:6, fontSize:10, color:"var(--accent3)", fontWeight:600 }}>
              ✅ Auto-saved to Saved Contacts
            </div>
          )}

          {/* Actions */}
          <div style={{ marginTop:6, display:"flex", gap:8, alignItems:"center" }}>
            {p.linkedin && <a href={p.linkedin} target="_blank" rel="noreferrer" style={{ fontSize:11, color:"var(--accent2)", fontWeight:600, textDecoration:"none" }}>🔗 LinkedIn</a>}
            {showEnrich && !p.enriched && !isEnriching && (
              <button onClick={()=>onEnrichSingle(p)} style={{
                fontSize:10, fontWeight:700, fontFamily:"var(--font)", border:"1px solid var(--accent)",
                borderRadius:5, padding:"2px 8px", cursor:"pointer", background:"#eff6ff", color:"var(--accent)",
              }}>🔍 Find Email & Phone</button>
            )}
          </div>
        </div>
      </div>
  ...
  // Pagination bar
  const Pagination = ({ page, total, perPage, onPage, searching }) => {
    const totalPages = Math.ceil(total / perPage) || 1;
    if (totalPages <= 1 && total <= perPage) return null;
    return (
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:12, fontSize:12, color:"var(--muted)" }}>
        <span>{total} total · Page {page} of {totalPages}</span>
        <div style={{ display:"flex", gap:6 }}>
          <button onClick={()=>onPage(page-1)} disabled={page<=1||searching} style={{ padding:"3px 10px", borderRadius:6, border:"1px solid var(--border)", background:"var(--surface)", cursor:page<=1?"not-allowed":"pointer", fontFamily:"var(--font)", fontSize:11, fontWeight:600, color:"var(--muted)" }}>← Prev</button>
          <button onClick={()=>onPage(page+1)} disabled={page>=totalPages||searching} style={{ padding:"3px 10px", borderRadius:6, border:"1px solid var(--border)", background:"var(--surface)", cursor:page>=totalPages?"not-allowed":"pointer", fontFamily:"var(--font)", fontSize:11, fontWeight:600, color:"var(--muted)" }}>Next →</button>
        </div>
      </div>
    );
  };
  ...
  const ResultsToolbar = ({ results, selected, onSelectAll, onClearAll, onEnrich, enriching, label }) => {
    const selCount = Object.values(selected).filter(Boolean).length;
    return (
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8, marginBottom:10 }}>
        <div style={{ fontSize:12, color:"var(--muted)", fontWeight:600 }}>
          {results.length} result{results.length!==1?"s":""} on this page
          {selCount > 0 && <span style={{ color:"var(--accent)", marginLeft:8 }}>· {selCount} selected</span>}
          {enriching && <span style={{ color:"var(--accent2)", marginLeft:8 }}>· enriching...</span>}
        </div>
        <div style={{ display:"flex", gap:6 }}>
          <button onClick={onSelectAll} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"3px 10px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>☑ All</button>
          {selCount > 0 && <button onClick={onClearAll} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"3px 10px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>✕ Clear</button>}
          <button onClick={onEnrich} disabled={enriching} style={{
            fontSize:11, fontWeight:700, fontFamily:"var(--font)", border:"none", borderRadius:6, padding:"4px 12px",
            cursor:enriching?"not-allowed":"pointer", background:enriching?"var(--border)":"var(--accent2)", color:"#fff",
            display:"inline-flex", alignItems:"center", gap:4,
          }}>
            {enriching ? <><Spinner size={11}/> Enriching...</> : `🔍 Find Email & Phone${selCount>0?` (${selCount})`:" (All)"}`}
          </button>
        </div>
      </div>
    );
  ...
  const modeBtn = (m, icon, label) => (
    <button onClick={()=>setMode(m)} style={{
      ...
      background:mode===m?"var(--accent)":"var(--surface)", color:mode===m?"#fff":"var(--muted)", boxShadow:"var(--shadow)",
    }}>{icon} {label}</button>
=======
...
>>>>>>> e5462fdddf70b12897fe31fbe39f9e53ea26f9da
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }} className="slide-in">
      <div>
<<<<<<< HEAD
        <h2 style={{ fontSize:22, fontWeight:800 }}>Apollo.io People Search</h2>
        <p style={{ color:"var(--muted)", fontSize:13, marginTop:4 }}>Search people by name or browse a company. Select people, then find emails &amp; phones — auto-saved to Saved Contacts.</p>
=======
      ...
      {mode === "person" && (
        <Card>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <div style={{ flex:1 }}>
              <Input placeholder="e.g. John Smith Stripe San Francisco" value={pq}
                onChange={e=>setPq(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchPeople(1)} />
=======
      {/* Mode toggle */}
      <div style={{ display:"flex", gap:8 }}>
...
>>>>>>> e5462fdddf70b12897fe31fbe39f9e53ea26f9da
            ...
            <button onClick={()=>searchPeople(1)} disabled={pSearching||!apolloKey} style={{
              background:pSearching||!apolloKey?"var(--border)":"var(--accent)", color:pSearching||!apolloKey?"var(--muted)":"#fff",
              border:"none", borderRadius:8, padding:"10px 20px", cursor:pSearching||!apolloKey?"not-allowed":"pointer",
              fontFamily:"var(--font)", fontWeight:700, fontSize:14, display:"inline-flex", alignItems:"center", gap:8,
              boxShadow:pSearching||!apolloKey?"none":"0 2px 8px rgba(37,99,235,.35)", transition:"all .15s", whiteSpace:"nowrap",
            }}>
              {pSearching ? <><Spinner size={15}/>Searching...</> : "🔍 Search"}
            </button>
          </div>

<<<<<<< HEAD
          {pError && <div style={{ marginTop:10, background:"#fee2e2", border:"1px solid #fca5a5", borderRadius:8, padding:"10px 14px", color:"var(--danger)", fontSize:12 }}>⚠ {pError}</div>}

          {pResults.length > 0 && (
            <div style={{ marginTop:16 }}>
              <ResultsToolbar
                results={pResults} selected={pSelected}
                onSelectAll={()=>setPSelected(pResults.reduce((a,_,i)=>({...a,[i]:true}),{}))}
                onClearAll={()=>setPSelected({})}
                onEnrich={enrichPersonLookup} enriching={bulkEnriching}
              />
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {pResults.map((p,i) => (
                  <PersonRow key={p.apollo_id||i} p={p} i={i} selected={pSelected}
                    onToggle={idx=>setPSelected(s=>({...s,[idx]:!s[idx]}))}
                    showEnrich={true}
                    onEnrichSingle={person=>bulkEnrich([person],"person_lookup",(results)=>{
                      setPResults(prev=>prev.map(r=>r.apollo_id===results[0]?.apollo_id?{...r,...results[0]}:r));
                    })}
                  />
                ))}
              </div>
              <Pagination page={pPage} total={pTotal} perPage={PER_PAGE} onPage={p=>searchPeople(p)} searching={pSearching} />
            </div>
          )}
        </Card>
      )}
      ...
      {mode === "company" && (
        <Card>
          <Label style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>🏢 Company People</Label>
          <p style={{ fontSize:12, color:"var(--muted)", marginBottom:14 }}>
            Find all people at a company. Filter by role, then select who you want to enrich.
          </p>

          {/* Search bar */}
          <div style={{ display:"flex", gap:10 }}>
            <div style={{ flex:1 }}>
              <Input placeholder="e.g. Stripe, Vercel, Linear..." value={cq}
                onChange={e=>setCq(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchCompany(1)} />
            </div>
            <button onClick={()=>searchCompany(1)} disabled={cSearching||!apolloKey} style={{
              background:cSearching||!apolloKey?"var(--border)":"var(--accent)", color:cSearching||!apolloKey?"var(--muted)":"#fff",
              border:"none", borderRadius:8, padding:"10px 20px", cursor:cSearching||!apolloKey?"not-allowed":"pointer",
              fontFamily:"var(--font)", fontWeight:700, fontSize:14, display:"inline-flex", alignItems:"center", gap:8,
              boxShadow:cSearching||!apolloKey?"none":"0 2px 8px rgba(37,99,235,.35)", transition:"all .15s", whiteSpace:"nowrap",
            }}>
              {cSearching ? <><Spinner size={15}/>Searching...</> : "🔍 Search"}
            </button>
          </div>
          ...
          <div style={{ marginTop:12, display:"flex", gap:6, flexWrap:"wrap" }}>
            {SENIORITY_GROUPS.map(g => {
              const active = g.values.length === 0 ? cSeniorities.length === 0 : g.values.every(v => cSeniorities.includes(v));
              return (
                <button key={g.label} onClick={()=>toggleSeniority(g)} style={{
                  fontSize:11, fontWeight:600, fontFamily:"var(--font)", borderRadius:20,
                  padding:"3px 12px", cursor:"pointer", transition:"all .15s",
                  background:active?g.color:"var(--surface)", color:active?"#fff":"var(--muted)",
                  border:`1px solid ${active?g.color:"var(--border)"}`,
                }}>{g.label}</button>
              );
            })}
            <span style={{ fontSize:11, color:"var(--muted)", alignSelf:"center", marginLeft:4 }}>
              {cSeniorities.length === 0 ? "Showing all roles" : `Filtered: ${cSeniorities.join(", ")}`}
            </span>
          </div>
          ...
          {cOrg && (
            <div style={{ marginTop:12, background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:8, padding:"10px 14px", display:"flex", gap:16, flexWrap:"wrap", fontSize:12 }}>
              ...
              {cOrg.employees && <span style={{ color:"var(--muted)" }}>👥 {cOrg.employees} employees</span>}
              {cOrg.industry  && <span style={{ color:"var(--muted)" }}>🏷 {cOrg.industry}</span>}
            </div>
          )}
          ...
          {cResults.length > 0 && (
            <div style={{ marginTop:14 }}>
              <ResultsToolbar
                results={cResults} selected={cSelected}
                onSelectAll={()=>setCSelected(cResults.reduce((a,_,i)=>({...a,[i]:true}),{}))}
                onClearAll={()=>setCSelected({})}
                onEnrich={enrichCompany} enriching={bulkEnriching}
              />
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {cResults.map((p,i) => (
                  <PersonRow key={p.apollo_id||i} p={p} i={i} selected={cSelected}
                    onToggle={idx=>setCSelected(s=>({...s,[idx]:!s[idx]}))}
                    showEnrich={true}
                    onEnrichSingle={person=>bulkEnrich([person],"company_lookup",(results)=>{
                      setCResults(prev=>prev.map(r=>r.apollo_id===results[0]?.apollo_id?{...r,...results[0]}:r));
                    })}
                  />
                ))}
              </div>
              <Pagination page={cPage} total={cTotal} perPage={PER_PAGE} onPage={p=>searchCompany(p)} searching={cSearching} />
            </div>
          )}
        </Card>
      )}
...
function SavedContactsView() {
  ...
  useEffect(() => { loadContacts(); }, [search, hasEmail, hasPhone, source, dateFrom, dateTo, sortBy, sortOrder]);
  ...
  const deleteSelected = async () => {
    ...
    setDeleting(true);
    try {
      await apiFetch("/contacts/bulk-delete", { method:"DELETE", body: JSON.stringify({ ids }) });
      setContacts(prev => prev.filter(c => !selected[c.id]));
      setSelected({});
    } catch(err) { alert(err.message); }
    setDeleting(false);
  };

  const exportCSV = () => { window.open(`${API_BASE}/contacts/export`, "_blank"); };

  const toggleSelect = (id) => setSelected(s => ({ ...s, [id]: !s[id] }));
  const selectAll    = () => setSelected(contacts.reduce((a,c) => ({ ...a, [c.id]: true }), {}));
  const clearAll     = () => setSelected({});
  const selectedCount = Object.values(selected).filter(Boolean).length;
  const copyText = (text, key) => { navigator.clipboard?.writeText(text); setCopied(key); setTimeout(()=>setCopied(null),1500); };

  const SortBtn = ({ field, label }) => (
    <button onClick={()=>{ if(sortBy===field) setSortOrder(o=>o==="asc"?"desc":"asc"); else { setSortBy(field); setSortOrder("desc"); } }} style={{
      fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"none", background:"transparent", cursor:"pointer",
      color:sortBy===field?"var(--accent)":"var(--muted)", display:"inline-flex", alignItems:"center", gap:2,
    }}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }} className="slide-in">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontSize:22, fontWeight:800 }}>Saved Contacts</h2>
          <p style={{ color:"var(--muted)", fontSize:13, marginTop:4 }}>Contacts auto-saved when you run enrichment. {count > 0 && <strong>{count} total</strong>}</p>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          {selectedCount > 0 && (
      ...
      <Card>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
          <div>
            <Label>Search</Label>
            <Input placeholder="Name, email, company..." value={search} onChange={e=>setSearch(e.target.value)} />
          </div>
          <div>
            <Label>Source</Label>
            <select value={source} onChange={e=>setSource(e.target.value)} style={{ width:"100%", padding:"8px 12px", borderRadius:8, border:"1px solid var(--border)", fontFamily:"var(--font)", fontSize:13, background:"var(--surface)", color:"var(--text)" }}>
              <option value="">All sources</option>
              <option value="person_lookup">Person Lookup</option>
              <option value="company_lookup">Company People</option>
            </select>
          </div>
          <div>
            <Label>Sort by</Label>
            <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{ width:"100%", padding:"8px 12px", borderRadius:8, border:"1px solid var(--border)", fontFamily:"var(--font)", fontSize:13, background:"var(--surface)", color:"var(--text)" }}>
          ...
          <div style={{ display:"flex", alignItems:"flex-end", gap:16, paddingBottom:2 }}>
            <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, fontWeight:600, cursor:"pointer" }}>
              <input type="checkbox" checked={hasEmail} onChange={e=>setHasEmail(e.target.checked)} style={{ accentColor:"var(--accent)" }} />
              Has Email
            </label>
            <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, fontWeight:600, cursor:"pointer" }}>
              <input type="checkbox" checked={hasPhone} onChange={e=>setHasPhone(e.target.checked)} style={{ accentColor:"var(--accent)" }} />
              Has Phone
            </label>
            <button onClick={()=>{ setSearch(""); setHasEmail(false); setHasPhone(false); setSource(""); setDateFrom(""); setDateTo(""); }} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"3px 10px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>
              Reset
            </button>
          </div>
        </div>
      ...
      <Card style={{ padding:0, overflow:"hidden" }}>
        ...
        <div style={{ padding:"10px 16px", borderBottom:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
          <div style={{ display:"flex", gap:6 }}>
            <button onClick={selectAll} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"3px 10px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>☑ All</button>
            {selectedCount > 0 && <button onClick={clearAll} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"3px 10px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>✕ Clear</button>}
          </div>
          <div style={{ display:"flex", gap:12, fontSize:11, color:"var(--muted)" }}>
            <SortBtn field="saved_at" label="Date" />
            <SortBtn field="name"     label="Name" />
            <SortBtn field="company"  label="Company" />
            <button onClick={()=>setSortOrder(o=>o==="asc"?"desc":"asc")} style={{ fontSize:11, fontWeight:600, fontFamily:"var(--font)", border:"1px solid var(--border)", borderRadius:6, padding:"2px 8px", cursor:"pointer", background:"var(--surface)", color:"var(--muted)" }}>
...
>>>>>>> e5462fdddf70b12897fe31fbe39f9e53ea26f9da
        ...
        {loading ? (
          <div style={{ padding:40, textAlign:"center", color:"var(--muted)", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
            <Spinner size={16} /> Loading contacts...
          </div>
        ) : contacts.length === 0 ? (
          <div style={{ padding:40, textAlign:"center", color:"var(--muted)" }}>
            <div style={{ fontSize:32, marginBottom:8 }}>📋</div>
            <div style={{ fontWeight:600 }}>No saved contacts yet</div>
            <div style={{ fontSize:12, marginTop:4 }}>Go to Apollo.io tab, search for people, and click "Find Email &amp; Phone" — they'll appear here automatically.</div>
          </div>
        ) : (
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
              <thead>
                <tr style={{ background:"var(--surface2)", borderBottom:"1px solid var(--border)" }}>
                  ...
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Name</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Title</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Company</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Email</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Phone</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Source</th>
                  <th style={{ padding:"8px 12px", textAlign:"left", fontWeight:700, color:"var(--muted)" }}>Saved</th>
                  <th style={{ padding:"8px 12px", width:40 }}></th>
                </tr>
              </thead>
              <tbody>
                {contacts.map((c, i) => (
                  <tr key={c.id} style={{ borderBottom:"1px solid var(--border)", background:selected[c.id]?"#eff6ff":i%2===0?"var(--surface)":"var(--surface2)" }}>
                    <td style={{ padding:"8px 12px" }}>
                      <input type="checkbox" checked={!!selected[c.id]} onChange={()=>toggleSelect(c.id)} style={{ accentColor:"var(--accent)", cursor:"pointer" }} />
                    </td>
                    <td style={{ padding:"8px 12px", fontWeight:600, whiteSpace:"nowrap" }}>
                      {c.name}
                      {c.linkedin && <a href={c.linkedin} target="_blank" rel="noreferrer" style={{ marginLeft:6, color:"var(--accent2)", fontSize:10 }}>🔗</a>}
                    </td>
                    <td style={{ padding:"8px 12px", color:"var(--muted)", maxWidth:160, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.title}</td>
                    <td style={{ padding:"8px 12px", whiteSpace:"nowrap" }}>
                      {c.company}
                      {c.domain && <div style={{ fontSize:10, color:"var(--muted)" }}>{c.domain}</div>}
                    </td>
                    <td style={{ padding:"8px 12px" }}>
                      {c.email ? (
                        <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                          <span style={{ fontFamily:"var(--mono)", color:"var(--accent)", fontSize:11 }}>{c.email}</span>
                          <button onClick={()=>{navigator.clipboard?.writeText(c.email);setCopied(`e-${c.id}`);setTimeout(()=>setCopied(null),1500);}} style={{ background:"transparent", border:"none", cursor:"pointer", color:copied===`e-${c.id}`?"var(--accent3)":"var(--muted)", fontSize:10 }}>
                            {copied===`e-${c.id}`?"✓":"📋"}
                          </button>
                        </div>
                    ...
                    <td style={{ padding:"8px 12px" }}>
                      {c.phone ? (
                        <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                          <span style={{ fontFamily:"var(--mono)", color:"#166534", fontSize:11 }}>{c.phone}</span>
                          <button onClick={()=>{navigator.clipboard?.writeText(c.phone);setCopied(`ph-${c.id}`);setTimeout(()=>setCopied(null),1500);}} style={{ background:"transparent", border:"none", cursor:"pointer", color:copied===`ph-${c.id}`?"var(--accent3)":"var(--muted)", fontSize:10 }}>
                            {copied===`ph-${c.id}`?"✓":"📋"}
                          </button>
                        </div>
                      ) : <span style={{ color:"var(--border)" }}>—</span>}
                    ...
                    <td style={{ padding:"8px 12px" }}>
                      <Badge variant={c.source==="person_lookup"?"info":"success"} style={{ fontSize:9 }}>
                        {c.source==="person_lookup"?"👤 Person":"🏢 Company"}
                      </Badge>
                    </td>
                    <td style={{ padding:"8px 12px", color:"var(--muted)", whiteSpace:"nowrap", fontSize:11 }}>
                      {new Date(c.saved_at).toLocaleDateString()}
                    </td>
                    <td style={{ padding:"8px 12px" }}>
                      <button onClick={()=>deleteOne(c.id)} title="Delete" style={{ background:"transparent", border:"none", cursor:"pointer", color:"var(--muted)", fontSize:13 }}>🗑</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
export default function App() {
  ...
  const NAV = [
    ...
    { id:"saved",     label:"Saved Contacts", icon:"📋" },
